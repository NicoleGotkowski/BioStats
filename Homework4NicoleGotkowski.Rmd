...
```{r}
#Question#5
data.frame(my.data.2)
model1<-glm(survival~age+Parents.children+Class+fare+Residence+Gender, data=my.variables)
summary.lm(model1)
```
```{r}
...
modelX2<-glm(lowbwt~smoker+motherweight+Gestation, data=my.variablesX.nona, family=binomial(link="logit"))
summary(modelX2)
```
```{r}
#Model2
data.frame(my.data.2)
model2<-glm(survived~Number.of.parents.children+Class+fare+Residence+Gender, data=my.data.2)
summary.lm(model1)
...
influenceIndexPlot(model1, id.n=8)
```
...
#Question10
# Based on the 
#Since the VIF <5, the variables are not correlated, therefore there is no multicollinearity  
```
...
my.data.2$survived=as.factor(my.data.2$survived)
train(survived ~Gender+age+Class+Residence+Number.of.parents.children,data=my.data.2, method="glm", family=binomial(link='logit'),
                 trControl = ctrl, tuneLength = 5)

```
```{r}
#Question 12
#Based on the k-fold, the model predicted 79% accurately, accuracy rate and the randomized model predicted 55% accurately. Therefore, the model was better than randomly selected data, meaning it was an ok model. 
```

```{r}
#Question13
confusionMatrix(data=as.factor(as.numeric(predictions>0.5)),reference=my.data.2$survived)
```
